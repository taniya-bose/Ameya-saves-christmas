...
/* =========================================================
   ...
   scoring animation (+1 pop & confetti)
   ========================================================= */

/* ---------- Config ---------- */
const PLAYER_IMAGE_SRC = 'ameya_head-removebg-preview.png'; // place the file next to index.html
const TOTAL_CANDY      = 5;
const TOTAL_TIME       = 90;
const GRAVITY          = 0.48;
const FRICTION         = 0.88;
...
// Image fit tweaks (adjust if the sprite looks small/high)
const SPRITE_SCALE_ADJ = 1.08; // >1 enlarges
const SPRITE_Y_ADJ     = 2;    // +down to sit on platforms
...
let checkpoint = { x: 30, y: 460 };
let collectedCount = 0;
const keys = {};

/* ---------- Festive visuals ---------- */
const snowflakes = [...Array(120)].map(()=>({
  x: Math.random()*960, y: Math.random()*540, r: Math.random()*2+0.6, s: Math.random()*0.6+0.3
}));
let phase = 0;
...
function bell(d=0.15,f=880){
  try{
    ...
    osc.start(); osc.stop(audioCtx.currentTime+d+0.02);
  }catch(e){}
}
...
function makeLevel(){
  ...
  candies = [
    { x:210, y:390 }, { x:470, y:320 }, { x:680, y:270 }, { x:120, y:230 }, { x:560, y:130 }
  ];
...
function loadPlayerImage(){
  ...
  img.onload = ()=>{
    ...
    const targetW = player.w, targetH = player.h;
    const s = Math.min(targetW / img.width, targetH / img.height);
    playerScale = s * SPRITE_SCALE_ADJ;
    playerOffsetY = targetH - img.height * s + SPRITE_Y_ADJ;
  };
  img.onerror = ()=>{
    console.warn("Player image failed to load—using fallback sprite.");
...
function drawEffects(){
  ...
  for(const c of confetti){
    ...
    ctx.fillStyle = c.color; ctx.fillRect(c.x, c.y, 3, 3); ctx.restore();
  }
}
...
function drawAurora(){
  ...
  grad.addColorStop(0, `rgba(121,237,255,${0.06+Math.sin(t)*0.03})`);
  grad.addColorStop(1, `rgba(255,150,200,${0.04+Math.cos(t*0.7)*0.03})`);
  ctx.fillStyle = grad; ctx.fillRect(0,0,960,160);
}
function drawSnow(){
  ctx.fillStyle = '#0b1626'; ctx.fillRect(0,0,960,540);
  drawAurora();
  for(const s of snowflakes){
    ctx.fillStyle = 'rgba(255,255,255,.75)';
    ctx.beginPath(); ctx.arc(s.x, s.y, s.r, 0, Math.PI*2); ctx.fill();
    s.y += s.s; s.x += Math.sin(phase + s.y*0.01) * 0.15;
    if(s.y>540){ s.y = -4; s.x = Math.random()*960; }
  }
}
...
function draw(){
  ...
  for (const wall of [leftWall, rightWall]) {
    ...
    ctx.fillStyle = '#284462'; ctx.fillRect(wall.x, wall.y+80, wall.w, 8);
    ctx.fillStyle = '#284462'; ctx.fillRect(wall.x, wall.y+160, wall.w, 8);
    ctx.fillStyle = '#284462'; ctx.fillRect(wall.x, wall.y+240, wall.w, 8);
    ctx.fillStyle = '#284462'; ctx.fillRect(wall.x, wall.y+320, wall.w, 8);
    ctx.fillStyle = '#284462'; ctx.fillRect(wall.x, wall.y+400, wall.w, 8);
    ctx.fillStyle = '#284462'; ctx.fillRect(wall.x, wall.y+480, wall.w, 8);
    const cx = wall === leftWall ? wall.x + wall.w - 2 : wall.x + 2;
    drawLightsStrip(cx, 40, 1, 1); drawLightsStrip(cx, 120, 1, 1);
    drawLightsStrip(cx, 200, 1, 1); drawLightsStrip(cx, 280, 1, 1);
    drawLightsStrip(cx, 360, 1, 1); drawLightsStrip(cx, 440, 1, 1);
  }
  ...
  for(const s of spikes){
    ...
    for(let i=0;i<s.w;i+=10){
      ctx.beginPath(); ctx.moveTo(s.x+i, s.y+s.h);
  ...
  // Finish area: sleigh + reindeer + tree
  ...
  drawLightsStrip(tx+10, ty+70, 60, 16);
  drawLightsStrip(tx, ty+130, 80, 16);

  // Candies
  for(const c of candies){
    const wiggle = Math.sin(phase*2 + c.x*0.01)*2;
    ctx.fillStyle='#ff4d4d';
    ctx.beginPath(); ctx.arc(c.x, c.y+wiggle, 8, 0, Math.PI*2); ctx.fill();
    ctx.strokeStyle='#fff'; ctx.lineWidth=2; ctx.beginPath();
    ctx.moveTo(c.x+6, c.y-6+wiggle); ctx.lineTo(c.x+12, c.y-12+wiggle); ctx.stroke();
  }
