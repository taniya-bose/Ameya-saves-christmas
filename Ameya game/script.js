...
/* =========================================================
   ...
   Clean build: visible candies, grounded tree, boundary walls,
   image sprite (with fallback), scoring animation
   ========================================================= */

/* ===== Config ===== */
const PLAYER_IMAGE_SRC = 'ameya_head-removebg-preview.png'; // file next to index.html
...
function makeLevel() {
  ...
  // in-bounds candies (then clamped to be extra-safe)
  candies = [
    { x: 210, y: 390 },
    { x: 470, y: 320 },
    { x: 680, y: 270 },
...
// Grounded Christmas tree (trunk touches groundTop)
function drawTreeOnGround(tx, groundTop = 500) {
  const TIER1 = 60, TIER2 = 70, TIER3 = 80, TRUNK_H = 16;
  const TREE_TOTAL = TIER1 + TIER2 + TIER3 + TRUNK_H;
  const ty = groundTop - TREE_TOTAL;
  ...
  ctx.fillRect(tx + 34, groundTop - TRUNK_H, 12, TRUNK_H);
...
function draw() {
  ...
  // Grounded tree at x=860 (ground top is 500)
  drawTreeOnGround(860, 500);

  // === Candies (draw AFTER environment) ===
