...
/* =========================================================
   Can you save Christmas, Ameya?
   Festive platformer with boundary walls, player image sprite,
   visible candies (correct draw order) + scoring animation,
   tree placed on ground
   ========================================================= */

/* ===== Config ===== */
const PLAYER_IMAGE_SRC = 'ameya_head-removebg-preview.png'; // file sits next to index.html
...
const SPRITE_SCALE_ADJ = 1.08, SPRITE_Y_ADJ = 2; // tweak if needed
const WIDTH = 960, HEIGHT = 540;                 // canvas internal resolution
...
let running = false, paused = false, timer = null, timeLeft = TOTAL_TIME;
let best = Number(localStorage.getItem('xmas_best') || 0);
let checkpoint = { x: 30, y: 460 }, collectedCount = 0;
const keys = {};

/* ===== Visuals ===== */
const snowflakes = [...Array(120)].map(() => ({
  x: Math.random() * WIDTH,
...
function clampToCanvas(x, y, margin = 14) {
  const XMIN = margin, YMIN = margin, XMAX = WIDTH - margin, YMAX = HEIGHT - margin;
  return { x: Math.max(XMIN, Math.min(XMAX, x)), y: Math.max(YMIN, Math.min(YMAX, y)) };
}
...
function makeLevel() {
  ...
  // Fixed, in-bounds candies (then clamped to be extra-safe)
  candies = [
    { x: 210, y: 390 },
    { x: 470, y: 320 },
    { x: 680, y: 270 },
    { x: 120, y: 230 },
    { x: 560, y: 130 },
  ].map(c => clampToCanvas(c.x, c.y));

  reindeer = { x: 900, y: 220, w: 40, h: 30 };

  // Visible boundary walls
  leftWall  = { x: -2,    y: 0, w: 10, h: HEIGHT };
  rightWall = { x: WIDTH - 8, y: 0, w: 10, h: HEIGHT };
}
...
function drawEffects() {
  for (const p of pops) {
    ...
    ctx.fillStyle = '#ffd166'; ctx.font = 'bold 18px system-ui';
    ctx.fillText('+1', p.x, p.y);
    ctx.restore();
  }
  for (const c of confetti) {
    ctx.save(); ctx.globalAlpha = Math.max(0, Math.min(1, c.life));
    ctx.fillStyle = c.color; ctx.fillRect(c.x, c.y, 3, 3);
    ctx.restore();
  }
}
...
function fullReset() {
  ...
  loadPlayerImage();
  draw(); // first frame with everything visible
}

function respawn() {
  player.x = checkpoint.x; player.y = checkpoint.y;
  player.vx = 0; player.vy = 0;
  player.onGround = false; player.coyote = COYOTE_FRAMES; player.doubleLeft = MAX_DOUBLE_JUMP;
}

function start() {
  if (running) return;
  running = true; overlayStart.hidden = true;
  timer = setInterval(() => {
    if (!paused) {
...
function gameWin() {
  ...
  finalTimeEl.textContent = timeLeft; overlayWin.hidden = false;
  bell(0.2, 1046); setTimeout(() => bell(0.2, 1318), 140);
...
function update() {
  ...
  for (const p of platforms) {
    if (collide(player, p)) {
      if (player.vy > 0) {
        ...
        player.coyote = COYOTE_FRAMES; player.doubleLeft = MAX_DOUBLE_JUMP;
  ...
  if (player.y > HEIGHT) respawn();

  // Candy collection (guarded)
  candies = candies.filter(c => {
    const hit = Math.abs(player.x + player.w / 2 - c.x) < 16 &&
               Math.abs(player.y + player.h / 2 - c.y) < 16;
    if (hit) {
...
function drawAurora() {
  ...
  g.addColorStop(0, `rgba(121,237,255,${0.06 + Math.sin(phase) * 0.03})`);
  g.addColorStop(1, `rgba(255,150,200,${0.04 + Math.cos(phase * 0.7) * 0.03})`);
  ctx.fillStyle = g; ctx.fillRect(0, 0, WIDTH, 160);
}
function drawSnow() {
  ctx.fillStyle = '#0b1626'; ctx.fillRect(0, 0, WIDTH, HEIGHT);
  drawAurora();
  for (const s of snowflakes) {
    ctx.fillStyle = 'rgba(255,255,255,.75)';
    ctx.beginPath(); ctx.arc(s.x, s.y, s.r, 0, Math.PI * 2); ctx.fill();
    s.y += s.s; s.x += Math.sin(phase + s.y * 0.01) * 0.15;
    if (s.y > HEIGHT) { s.y = -4; s.x = Math.random() * WIDTH; }
  }
}
...
// Draw a Christmas tree anchored to the ground top (500)
function drawTreeOnGround(tx, groundY = 500) {
  const TIER1 = 60, TIER2 = 70, TIER3 = 80, TRUNK_H = 16;
  const TREE_TOTAL = TIER1 + TIER2 + TIER3 + TRUNK_H;
  const ty = groundY - TREE_TOTAL;

  ctx.fillStyle = '#2e6b3a';
  // Top tier
  ctx.beginPath(); ctx.moveTo(tx + 40, ty); ctx.lineTo(tx, ty + TIER1); ctx.lineTo(tx + 80, ty + TIER1);
  ctx.closePath(); ctx.fill();
  // Middle tier
  const midTop = ty + TIER1 - 10;
  ctx.beginPath(); ctx.moveTo(tx + 40, midTop); ctx.lineTo(tx - 20, midTop + TIER2); ctx.lineTo(tx + 100, midTop + TIER2);
  ctx.closePath(); ctx.fill();
  // Bottom tier
  const botTop = midTop + TIER2 - 10;
  ctx.beginPath(); ctx.moveTo(tx + 40, botTop); ctx.lineTo(tx - 40, botTop + TIER3); ctx.lineTo(tx + 120, botTop + TIER3);
  ctx.closePath(); ctx.fill();

  // Trunk touching ground
  ctx.fillStyle = '#8b5a2b';
  ctx.fillRect(tx + 34, groundY - TRUNK_H, 12, TRUNK_H);

  // Star + lights on tiers
  ctx.fillStyle = '#ffd166';
  ctx.beginPath(); ctx.arc(tx + 40, ty - 6, 6, 0, Math.PI * 2); ctx.fill();
  drawLightsStrip(tx + 10, midTop + TIER2 * 0.45, 60, 16);
  drawLightsStrip(tx,      botTop + TIER3 * 0.55, 80, 16);
}
...
function draw() {
  ...
  for (const wall of [leftWall, rightWall]) {
    ...
    ctx.fillStyle = '#284462';
    ctx.fillRect(wall.x, wall.y, wall.w, 12);
    for (let y = 80; y <= HEIGHT - 60; y += 80) { ctx.fillRect(wall.x, y, wall.w, 8); }
  ...
  // Platforms + lights
  for (const p of platforms) {
    rect(p.x, p.y, p.w, p.h, '#244466');
    ctx.fillStyle = '#2e5985'; ctx.fillRect(p.x, p.y, p.w, 6);
  ...
  // Tree anchored to ground (at x=860; ground top = 500)
  drawTreeOnGround(860, 500);

  // === Candies (draw AFTER environment so they stay visible) ===
  for (const c of candies) {
    const wiggle = Math.sin(phase * 2 + c.x * 0.01) * 2;

    // Body (red disc) + white ring for strong contrast
    ctx.beginPath();
    ctx.arc(c.x, c.y + wiggle, 10, 0, Math.PI * 2);
    ctx.fillStyle = '#ff3b3b'; ctx.fill();
    ctx.lineWidth = 2.5; ctx.strokeStyle = '#ffffff'; ctx.stroke();

    // Candy cane stripe
    ctx.beginPath();
    ctx.moveTo(c.x + 6, c.y - 5 + wiggle);
    ctx.lineTo(c.x + 13, c.y - 12 + wiggle);
...
function tryJump() {
  ...
  if (canGround) { player.vy = -JUMP_POWER; player.onGround = false; player.coyote = 0; player.doubleLeft = MAX_DOUBLE_JUMP; return; }
